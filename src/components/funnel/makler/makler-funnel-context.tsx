...
import type { StateData, UTMParameters } from "./makler-types";
...
export const MaklerFunnelProvider = ({
	children,
}: {
	...
	// Debug
	useEffect(() => {
		console.log("MaklerFunnelProvider data updated:", data);
	}, [data]);
...
export function useMaklerFunnel(): UseFunnelContextType {
	const value = useContext(FunnelContextType);

	if (!value) {
		throw new Error("Must be used inside MaklerFunnelProvider");
	}
