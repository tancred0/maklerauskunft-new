...
> = ({ index, item, onclick, variant = "default" }) => {
	...
	return (
		<button
			className={`flex h-16 w-full items-center justify-between rounded-lg border border-border bg-white px-4 ${isSmall ? "py-2" : "py-3"} mb-2 transition-colors hover:bg-primary/10 hover:border-primary`}
			key={index}
			onClick={() => onclick()}
		>
